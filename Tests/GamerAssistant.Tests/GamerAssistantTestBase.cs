...
using GamerAssistant.Migrations.SeedData;
...
{
    public abstract class GamerAssistantTestBase : AbpIntegratedTestBase<GamerAssistantTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected GamerAssistantTestBase()
        {
        ...
        protected void UsingDbContext(Action<GamerAssistantDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<GamerAssistantDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<GamerAssistantDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<GamerAssistantDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<GamerAssistantDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<GamerAssistantDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<GamerAssistantDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<GamerAssistantDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<GamerAssistantDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<GamerAssistantDbContext, Task<T>> func)
        {
            T result;
